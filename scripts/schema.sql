...
CREATE TABLE IF NOT EXISTS film_videos (
  ...
  -- Trailer/Teaser/Showreel/Behind the Scenes/Interview/Official Video/Clip.
  video_type VARCHAR(30) NOT NULL DEFAULT 'Trailer',
  -- Only set for directly-uploaded (non-YouTube/Vimeo) videos, chosen from
  -- the Media Library.
  poster_url VARCHAR(300) NULL,
  sort_order INT NOT NULL DEFAULT 0,
  FOREIGN KEY (film_id) REFERENCES films(id) ON DELETE CASCADE
...
-- Central Media Library: every uploaded image/video, reused across
-- Films/Books/Gallery via URL references. See src/lib/mediaRepo.ts.
